...
export default class App extends Component {
  ...
  handledoneAdd(i) {
    ...
    e.preventDefault();
    // フォームから受け取ったデータをオブジェクトに挿入してstateのtodo配列に追加
    this.state.done.push({ title: this.state.todo.pop(i) }); // まだ保存されていない
    // setStateを使ってstateを上書き
      ...
      <div className="siimple-grid">
        <div className="siimple-box siimple--bg-primary">
          ...
          <div className="siimple-grid-row">
            <div className="siimple-grid-col siimple-grid-col--5">
              <h2 className="siimple-box-title siimple--color-light">Todo</h2>
              <List todos={this.state.todo} handleRemove={this.handle} />
            </div>
            <div className="siimple-grid-col siimple-grid-col--5">
              <h2 className="siimple-box-title siimple--color-light">Done</h2>
              <DoneList dones={this.state.done} handleRemove={this.handleRemove} />
            </div>
          </div>
        </div >
      </div >
